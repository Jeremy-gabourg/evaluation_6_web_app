...
class SafeHouseType
{
    ...
    public function removeSafeHouseTypes(int $typesId): void
    {
        try {

            include (__DIR__.'/../controleurs/bdd_connexion.php');

            $sql2='DELETE FROM safe_houses_types WHERE id = :id';
    ...
    public function displaySafeHouseTypesList (): void
    {
        try {
            ...
            $sql = 'SELECT COUNT(*) AS nb_types FROM safe_houses_types';
            $statement1 = $pdo->prepare($sql);
            $statement1->execute();
            $result = $statement1->fetch();

            $nb_safe_houses_types = $result['nb_types'];
            $nbParPage = 5;
            $nbPages = ceil($nb_safe_houses_types/$nbParPage);
            $start = ($currentpage*$nbParPage)-$nbParPage;

            $sql2 = 'SELECT * FROM safe_houses_types ORDER BY name ASC  LIMIT :start, :nbParPage';
            $statement2 = $pdo->prepare($sql2);
            ...
            if ($statement2->execute()) {
                while($types = $statement2->fetchObject('SafeHouseType')){
                    echo     '<tr class="align-middle">
                                                <th scope="row">'.$types->getId().'</th>
                                                <td>'.$types->getName().'</td>
                                                <td class="text-end">
                                                    <form method="post" action="/controleurs/modify_safe_house_type.php" class="d-inline-block">
                ...
                echo '   
                            ...
                            <div class="text-center mt-4">
                                <a role="button" class="btn btn-success" href="/controleurs/add_safe_house_type.php">
                ...
                if($nb_safe_houses_types>$nbParPage){
                    echo '
    ...
    public function displaySelectedType(int $safeHouseTypeId): void
    {
        try {
            $sql = 'SELECT * FROM safe_houses_types WHERE id=:id';

            include (__DIR__.'/../controleurs/bdd_connexion.php');
            $statement=$pdo->prepare($sql);
            $statement->bindParam('id', $safeHouseTypeId, PDO::PARAM_INT);
            if($statement->execute()) {
                while ($type = $statement->fetchObject('SafeHouseType')) {
                    require_once (__DIR__.'/../vues/back_template.html');
                    require_once (__DIR__ . '/../vues/modify_safe_houses_types_form.php');
                    $_SESSION['safeHousetypeId'] = $safeHouseTypeId;
                }
                echo '
                </main>
                </div>
    ...
    public function modifyType($safeHouseTypeId): void
    {
        try {
            require_once (__DIR__.'/../controleurs/bdd_connexion.php');

            $this->setId($safeHouseTypeId);
            $this->setName($_POST['name']);

            $sql='UPDATE safe_houses_types SET name=:name WHERE id=:id';
            $statement = $pdo->prepare($sql);
    ...
    public function addType(): void
    {
        if ($_POST['name']!=="") {
            ...
            try {
                ...
                $sql = 'INSERT INTO safe_houses_types(name) VALUES (:name)';
                $statement = $pdo->prepare($sql);
