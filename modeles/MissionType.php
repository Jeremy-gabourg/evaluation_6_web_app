...
class MissionType
{
    ...
    public function removeMissionTypes(int $missionTypeId): void
    {
        try {

            include (__DIR__.'/../controleurs/bdd_connexion.php');

            $sql2='DELETE FROM mission_types WHERE id = :id';
            $statement = $pdo->prepare($sql2);

            $statement->bindParam('id', $missionTypeId, PDO::PARAM_INT);
    ...
    public function displayMissionTypesList (): void
    {
        try {
            ...
            $sql = 'SELECT COUNT(*) AS nb_types FROM mission_types';
            $statement1 = $pdo->prepare($sql);
            $statement1->execute();
            $result = $statement1->fetch();

            $nb_mission_types = $result['nb_types'];
            $nbParPage = 5;
            $nbPages = ceil($nb_mission_types/$nbParPage);
            $start = ($currentpage*$nbParPage)-$nbParPage;

            $sql2 = 'SELECT * FROM mission_types ORDER BY name ASC  LIMIT :start, :nbParPage';
            ...
            if ($statement2->execute()) {
                while($types = $statement2->fetchObject('MissionType')){
                    echo     '<tr class="align-middle">
                                                <th scope="row">'.$types->getId().'</th>
                                                <td>'.$types->getName().'</td>
                                                <td class="text-end">
                                                    <form method="post" action="/controleurs/modify_mission_type.php" class="d-inline-block">
                ...
                echo '   
                            ...
                            <div class="text-center mt-4">
                                <a role="button" class="btn btn-success" href="/controleurs/add_mission_type.php">
                ...
                if($nb_mission_types>$nbParPage){
                    echo '
                        <nav aria-label="Page navigation mt-4 bg-dark">
                          <ul class="pagination justify-content-center mt-4">
                             <li class="page-item ';if($currentpage == 1){echo'disabled';} else{"";};echo'"><a class="page-link" href="mission_types_listing.php/?page='.($currentpage-1).'">Precédente</a></li>';
                    for($page=1;$page<=$nbPages;$page++){
                        echo'<li class="page-item ';if($currentpage==$page){echo'active';}else{"";};echo'"><a class="page-link" href="mission_types_listing.php/?page='.$page.'">'.$page.'</a></li>';
                    }
                    echo'<li class="page-item ';if($currentpage == $nbPages){echo'disabled';}else{"";};echo'"><a class="page-link" href="mission_types_listing.php/?page='.($currentpage+1).'">Suivante</a></li>
                          </ul>
                        </nav>
                        </main>
                        </body>
    ...
    public function addType(): void
    {
        if ($_POST['name']!=="") {
            ...
            try {
                ...
                $sql = 'INSERT INTO mission_types(name) VALUES (:name)';
    ...
    public function displaySelectedType(int $missionTypeId): void
    {
        try {
            $sql = 'SELECT * FROM mission_types WHERE id=:id';
            ...
            if($statement->execute()) {
                while ($type = $statement->fetchObject('MissionType')) {
                    ...
                    $_SESSION['missionTypeId'] = $missionTypeId;
                }
                echo '
                </main>
                </div>
    ...
    public function modifyType($missionTypeId): void
    {
        try {
            require_once (__DIR__.'/../controleurs/bdd_connexion.php');

            $this->setId($missionTypeId);
            $this->setName($_POST['name']);

            $sql='UPDATE mission_types SET name=:name WHERE id=:id';
            $statement = $pdo->prepare($sql);
